...
In a typical week, Alex supports the fundraising team as they enter and classify donation, pledges, and donor touchpoints. Alex also manages FTC’s multiple email lists and coordinates the award-winning program, Tiny Tots for Tiny Creeks, which involves managing registration forms, safety releases, and sophisticated program management. Alex has a full plate! Can you relate?
...
*As we've seen, there are significant limitations with Partial Copy sandboxes. But if Alex wants to test and train with data in a sandbox, they need data! Alex did thorough research to identify other ways to create data. Each option has pros and cons, and requires different skills and competencies. Alex labeled each option with Admin, Developer, and Consultant to indicate the required skill set.*
...
### Third-Party Apps ###
There are a number of popular third-party apps available to help you load data into Salesforce. Check out Apsona, CloudDingo, or look for your own app to use.
...
*After all this work to craft a useful data set with related records, Alex wants to preserve it so that they can use it again in the future. Let's explore their options!*
